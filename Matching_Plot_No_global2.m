...
C_W1=Corresp(Flag==0,:);   % 错误的匹配点-红色
    if ~isempty(C_Neg)
        line([loc1(C_Neg(:,1),2)'; loc2(C_Neg(:,2),2)'+cols1+interval], [loc1(C_Neg(:,1),1)' ; loc2(C_Neg(:,2),1)'],'linewidth', 1, 'color', 'w', 'Marker', 'x', 'MarkerEdgeColor','y') ;%'g'
    end


    if ~isempty(C_C1)
        C_C=C_C1((1:round(5*end/6)),:);
        line([loc1(C_C(:,1),2)'; loc2(C_C(:,2),2)'+cols1+interval], [loc1(C_C(:,1),1)' ;  loc2(C_C(:,2),1)'],'linewidth', 1, 'color','b', 'Marker', 'x', 'MarkerEdgeColor','y' ) ;%[0,0.5,0.8]
    end
    
    if ~isempty(C_W1)
        C_W=C_W1((1:round(3*end/5)),:);
        line([loc1(C_W(:,1),2)'; loc2(C_W(:,2),2)'+cols1+interval], [loc1(C_W(:,1),1)' ;  loc2(C_W(:,2),1)'],'linewidth', 1, 'color','r', 'Marker', 'x', 'MarkerEdgeColor','y') ;%  [0.8,0.1,0]
    end
